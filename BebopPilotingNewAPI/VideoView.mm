    ...
    if (_canDisplayVideo)
    {
        if (!_shouldWaitForIFrame || frame->isIFrame)
        {
            ...
            if (!failed && frame->isIFrame)
            {
                ...
                sps = reinterpret_cast<uint8_t *>(malloc(_spsSize-4));
                pps = reinterpret_cast<uint8_t *>(malloc(_ppsSize-4));
            ...
            if (!failed && frame->isIFrame)
            {
                ...
                size_t sizes[] = {static_cast<size_t>(_spsSize-4), static_cast<size_t>(_ppsSize-4)};
            ...
            if (!failed)
            {
                if (frame->isIFrame)
                {
                    ...
                    data = reinterpret_cast<uint8_t *>(malloc(blockLength));
                }
                else
                {
                    offset = 0;
            ...
            if (!failed)
            {
                data = reinterpret_cast<uint8_t *>(memcpy(data, &frame->data[offset], blockLength));
